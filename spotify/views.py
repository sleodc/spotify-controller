...
class AuthURL(APIView):
	def get(self,request,format=None):
		scopes = 'user-read-playback-state user-modify-playback-state user-read-currently-playing playlist-modify-public'
...
class AddSong(APIView):
	def post(self, request, format=None):
		...
		if self.request.session.session_key:
			add_song(room.host, playlist, song)
...
class GetPlaylist(APIView):
	def post(self, request, format=None):
		...
		playlist = request.data.get('playlist')
		if self.request.session.session_key:
			playlist = get_playlist(room.host, playlist)
